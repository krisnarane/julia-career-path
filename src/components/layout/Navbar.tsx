  ...
  return (
    <header className="sticky top-0 z-40 glass border-b border-border">
      <nav className="max-w-6xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          ...
          {links.map((l) => (
            <li key={l.to}>
              <Link
                ...
                activeProps={{ className: "px-3 py-2 rounded-full text-sm font-medium gradient-primary text-white shadow-soft" }}
        ...
        <button className="md:hidden p-2 rounded-lg hover:bg-secondary" onClick={() => setOpen((v) => !v)} aria-label="Menu">
            ...
            {links.map((l) => (
              <li key={l.to}>
                <Link
                  ...
                  activeProps={{ className: "block px-3 py-2 rounded-lg text-sm font-medium gradient-primary text-white" }}
                  activeOptions={{ exact: true }}
                >
                  {l.label}
                </Link>
